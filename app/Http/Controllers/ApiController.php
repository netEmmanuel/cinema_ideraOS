...
class ApiController extends Controller
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    protected function respondWithTransformer($data, $includes = null, $statusCode = 200, $headers = [])
    {
        $this->checkTransformer();

        if ($data instanceof Collection) {
            $data = $this->collection($data, new $this->transformer, $includes);
        } else {
            $data = $this->item($data, new $this->transformer, $includes);
        }
    ...
    {
        if ($this->transformer === null || ! $this->transformer instanceof TransformerAbstract) {
